...
| BR6 -> S (S (S (S (S (S O)))))
| BR7 -> S (S (S (S (S (S (S O))))))
| BR8 -> S (S (S (S (S (S (S (S O)))))))
| BR9 -> S (S (S (S (S (S (S (S (S O))))))))
| BR10 -> S (S (S (S (S (S (S (S (S (S O)))))))))
        ...
        | Some ins0 ->
          let st1 =
            step Int64.zero ins0 rm m init_stack_state sv init_func_map true
              (Int64.repr (Zpos (XO (XO (XO (XO (XO (XO (XO (XO (XO (XO (XO
          ...
          then (match st1 with
                | BPF_OK (pc1, rm1, _, _, _, _, _, _) ->
                  (&&) (Int64.eq pc1 ipc)
                    (Int64.eq (eval_reg (int64_to_bpf_ireg i) rm1) res)
                | _ -> false)
               ...
               then (match st1 with
                     | BPF_OK (pc1, rm1, m2, ss1, sv1, fm1, _, _) ->
                       (match rbpf_decoder (S O) lp with
                        | Some ins1 ->
                          (match step pc1 ins1 rm1 m2 ss1 sv1 fm1 true
                                   (Int64.repr (Zpos (XO (XO (XO (XO (XO (XO
                                     (XO (XO (XO (XO (XO (XO (XO (XO (XO (XO
                                     (XO (XO (XO (XO (XO (XO (XO (XO (XO (XO
                                     (XO (XO (XO (XO (XO (XO
                                     XH))))))))))))))))))))))))))))))))))
                                   Int64.one (Int64.add Int64.one Int64.one) b with
                           | BPF_OK (pc2, rm2, _, _, _, _, _, _) ->
                             (&&) (Int64.eq pc2 ipc)
                               (Int64.eq (eval_reg (int64_to_bpf_ireg i) rm2)
                                 res)
                           | _ -> false)
                        | None -> false)
                     | _ -> false)
