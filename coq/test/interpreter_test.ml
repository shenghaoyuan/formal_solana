...
(** val init_bpf_state : reg_map -> Mem.mem -> z -> sBPFV -> bpf_state **)
...
(** val bpf_interp :
    nat -> bpf_bin -> bpf_state -> bool -> z -> block -> bpf_state **)
...
let bpf_interp_test lp lm _ v fuel res is_ok =
  let st1 =
    bpf_interp (Z.to_nat (Int64.unsigned (Int64.add fuel Int64.one))) lp
      (init_bpf_state init_reg_map
        (byte_list_to_mem (int64_to_byte_list lm) Mem.empty XH Z0)
        (Int64.add fuel Int64.one) (if Int64.eq v Int64.one then V1 else V2))
      true
